CREATE USER sql_user WITH PASSWORD 'teste';
...
SELECT 
    ...
    ROUND(AVG(v.valor), 2) AS "Media Salarial",
    MAX(v.valor) AS "Maior Salario",
    MIN(v.valor) AS "Menor Salario"
FROM 
    departamento d
LEFT JOIN 
    divisao div ON d.cod_dep = div.cod_divisao
LEFT JOIN 
    emp_venc ev ON div.cod_divisao = ev.cod_venc
LEFT JOIN 
    vencimento v ON ev.cod_venc = v.cod_venc
LEFT JOIN 
    empregado e ON ev.matr = e.matr
GROUP BY 
    d.nome
ORDER BY 
    AVG(v.valor) DESC;
